...
function wg_get_resource_dir($uniqid)
{
	...
	for( $i=0; $i<=1; $i++ )
	{
		...
		{
			if(!@mkdir($fullpath,0777)) return false;
			if(!@chmod($fullpath,0777)) return false;
		}
		else if(!is_dir($fullpath)) return false;
		else if(!is_writable($fullpath)) return false;
	}
...
/**
 * リソースの保存先ディレクトリを消去する。
 * @param $uniqid リソースフォルダ文字列
 * @return Boolean 成功したらtrueを、失敗したらfalseを返す。
 */
function wg_clean_resource_dir($uniqid)
{
	if( ($dir=wg_get_resource_dir($uniqid))===false ) return false;
	if( ($fd=@opendir($dir))===false ) return false;

	while(($file=@readdir($fd))!==false)
	{
		$fullpath = "{$dir}/{$file}";
		wg_printf("Checking => %s\n",$fullpath);
		if(is_file($fullpath))
		{
			wg_printf("Deleting => %s\n",$fullpath);
			@unlink($fullpath);
			clearstatcache();
			if(file_exists($fullpath)) {closedir($fd); return false;}
		}
	}
	closedir($fd);
	return true;
